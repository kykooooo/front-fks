...
## 6. ERRATA — contre-vérification adversariale du 02/09/2026

Le constat (cartographie, P0/P1/P2, comptages, annexes B et C, tests) a été rejoué par un second agent : chaque chiffre reproductible est tombé juste (785/20/0, 11 contrastes au centième, enums, rules, format de code). La partie PRESCRIPTION comportait deux erreurs de fond. L'ancien texte reste lisible plus haut.

**Erratum 1 — L'entrée coach existe, mais elle est INATTEIGNABLE après le premier lancement.** ~~« Elle existe déjà, il suffit de vérifier en 10 s sur un téléphone à jour. »~~ Le lien est bien sur Welcome (`WelcomeScreen.tsx:233-242`, sans flag), mais aucun `navigate("Welcome")` n'existe dans le dépôt : la seule porte est `RootNavigator.tsx:650` `initialRouteName={welcomeDone ? "Login" : "Welcome"}`, et `fks_welcome_done` n'est effacé que par la suppression de compte (`services/accountDeletionHelpers.ts:25`) ou `DEV_FLAGS.FORCE_WELCOME` (= false). Les trois CTA de Welcome posent le drapeau AVANT même qu'un compte existe. → Sur tout téléphone ayant déjà ouvert l'app une fois, l'entrée coach a disparu pour toujours. **Reclassé P1 (défendable P0 pilote)** : c'est la vraie réponse à la question de Kyllian, et le §5 « à confirmer par lui en 10 secondes » ne pouvait pas fonctionner. Corollaire : P1-02 (intention coach en mémoire React seulement, `RootNavigator.tsx:422`) est plus grave que décrit : l'intention meurt ET la porte est refermée.

**Erratum 2 — Le `writeBatch` recommandé pour P1-03 CASSERAIT la création de club.** ~~« Lot B en version A-mini : front-only, aucune rule, writeBatch pour l'atomicité. »~~ `userClubIdIsLegitimate()` (`firestore.rules:429-434`) appelle `isActiveMember()` → `myAccessRole()` qui fait `exists()`/`get()` sur `clubs/{clubId}/members/{uid}` ; dans un batch, les règles évaluent chaque opération contre l'état ANTÉRIEUR au batch → le membre créé dans le même batch est invisible → l'écriture `users/{uid}.clubId` est refusée → batch tout-ou-rien → aucun coach ne peut plus créer de club. Les règles le disent elles-mêmes (`:419-424` « écrit l'appartenance propriétaire JUSTE AVANT ce champ »), et ce document les cite en §2.4 avant de se contredire en §3.2/§3.3. Le seul correctif atomique est une **Cloud Function** (revue sécurité obligatoire) ; l'alternative front honnête est l'**idempotence** (réutiliser un `clubId` généré et persisté localement à chaque réessai, et relire `users/{uid}.clubId` au démarrage), pas un batch. Aucune Cloud Function de création de club n'existe aujourd'hui (`functions/src/index.ts`).

**Erratum 3 — « Anti double-tap partout » (§2.4) est faux pour `CoachOnboardingScreen`** : `loading` n'est posé que dans `doCreate` APRÈS l'Alert (`:78-103`) → deux taps rapides = deux Alerts = deux clubs. À joindre au correctif de P1-03.

**Erratum 4 — §5 « dosage backend sur `ageCategory` null » est fermable** : `getAgeCategoryCaps(null)` renvoie `null` (`readiness3 src/fksConfig.ts:411-414`), `fksOrchestrator.ts:4348` n'applique alors AUCUN plafond d'âge (ni familles interdites, ni volume, ni contacts plyo, ni sprint, ni durée) et l'audience retombe sur « amateur ». P1-04 (coach « Je m'entraîne aussi » → profil vide, `age_category: null`) = zéro protection, pas « un dosage par défaut ».

**Reclassements** : P0-01 (code club refusé) → P1-haut (le toast est titré « club non rejoint », l'onglet Profil mène bien à Paramètres → Club ; la matérialité — joueur invisible du coach, aucune trace persistante, aucun indicateur « pas de club » sur Home/Profil — tient). P2-05 (valeur accentuée `objectives[2]`) reste P2 : aucune allowlist d'objectif côté Functions/rules (vérifié).

**Vétilles** : dérive de références (`RootNavigator` +3 : 425→422, 716→713, 764→761 ; `clubsRepo` +7 à +10 ; `RegisterScreen:188`→189) ; compteurs par suite de l'annexe A (7/8/22/29) ne correspondent à aucune métrique mesurable ; « 8 événements » vs « 14 événements » = deux périmètres non expliqués ; `coachEntryIntent` rend aussi `CoachOnboardingScreen`.

---

## 7. DÉCISIONS — 02/09/2026 (orchestrateur, sur délégation explicite de Kyllian du 01/09)

**Lot A — front-only, lancé immédiatement** (une branche `fix/inscription-lot-a`) :
1. **Entrée coach toujours atteignable** : lien « Vous êtes coach ? » sur **Login** ET **Register** (pas seulement Welcome), qui pose l'intention coach ; **intention persistée en AsyncStorage** (clé dédiée, consommée à l'arrivée sur CoachOnboarding, effacée au logout/suppression de compte) → survit à une app tuée et au relogin ; le lien de secours de l'étape 1 du setup reste.
2. **Code club refusé = écran/carte persistante, pas un toast** : après la sauvegarde du profil, si le rattachement échoue → carte honnête sur place (« Ton profil est enregistré. Le code club n'a pas été reconnu. ») avec « Réessayer le code » / « Plus tard » ; **indicateur permanent** dans l'onglet Profil : rangée « Mon club » (« Aucun club — rejoindre avec un code » quand `clubId` absent) ; si le callable renvoie `coachAccess: "pending"` → texte « Demande envoyée, en attente de validation du coach » au lieu de « Tu as rejoint {club} » (si le champ n'est pas dans la réponse du callable : le dire, ne pas inventer).
3. **Coach « Je m'entraîne aussi »** : garde de complétude JOUEUR dans le portillon (profil `profileCompleted:true` mais champs joueur obligatoires absents → `ProfileSetupGate`, en préservant role/clubId/accessRole) ; **garde à la génération** : `ageCategory` absent → pas d'appel backend, message « Complète ton profil pour des séances adaptées à ta catégorie » + bouton vers le setup (justification : sans catégorie, aucun plafond d'âge ne s'applique côté moteur).
4. **Création de club robuste** : `loading` posé AVANT l'Alert (fin du double-tap) ; **idempotence** : `clubId` généré et persisté localement avant la 1re écriture, réutilisé à chaque réessai ; au timeout, message honnête (« La création a peut-être abouti, on vérifie ») et relecture de `users/{uid}.clubId` au démarrage suivant. PAS de `writeBatch` (erratum 2).
5. **Analytics** : `app.config.js` lit `process.env.EXPO_PUBLIC_AMPLITUDE_API_KEY` (même idiome que Sentry) ; ajout de `register_failed` ; la CLÉ elle-même = geste Kyllian (EAS secret / `.env.local`), documenté dans le rapport, aucune clé dans le dépôt.
6. **P2 rapides** : `autoComplete="given-name"` + `textContentType="givenName"` sur les prénoms, `autoComplete="off"` + `autoCapitalize="characters"` sur le code club ; mapping Firebase complété (`operation-not-allowed`, `user-disabled`, `invalid-credential` côté Register) ; prénom de Register pré-rempli et non redemandé au setup s'il existe ; valeur `objectives[2]` désaccentuée à l'ÉCRITURE + normalisation à la LECTURE (les profils existants gardent l'ancienne valeur en base — jamais de migration en masse) ; `maxFontSizeMultiplier` sur les 5 écrans du parcours ; force du mot de passe = longueur + variété (pas de faux « Fort » sur `aaaaaaaaaa`).
   Hors lot A (assumé) : remplacement du `SafeAreaView` de ProfileSetup par `<Screen>` (refonte visuelle, risque de régression tactile — chantier séparé), barrière d'âge < 13 ans (décision produit), CTA sombre 2,61:1 (piège thème sombre global déjà consigné).

**Lot B — Cloud Function `createClubAsCoach` atomique + parcours adjoint/second coach** : nécessite functions + rules → revue sécurité obligatoire → **après le lot A et sur go explicite de Kyllian** (touche la frontière coach-safe et le déploiement manuel des règles).

**Lot C — funnel analytics complet (events manquants entre Welcome et 1re génération, cf. `origin/claude/onboarding-lot0-analytics-afd479`) + finitions** : après recette téléphone du lot A.
