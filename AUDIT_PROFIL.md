...
**Bilan des vérifications : 39 accusations instruites, 39 confirmées à la première passe, 0 réfutée.** Trois défauts ont été trouvés indépendamment par deux chasseurs différents (convergence).

**Re-échantillonnage du 07/08 (réserve de méthode)** — un taux de 39/39 pouvant trahir des vérificateurs complaisants, 3 P0 ont été rejoués par des contre-vérificateurs frais dont la mission était de **réfuter**, pas de confirmer. Résultat : la passe a mordu. P0-2 confirmé (avec deux nuances de portée), P0-4 confirmé **et aggravé**, **P0-6 rétrogradé P1** (la moitié « systématique » de l'accusation est fausse — voir sa fiche), et un sous-grief du P0-2 **blanchi** (le `?? 0` du graphe d'intensité est un zéro vrai, pas une fabrication). Après ces corrections : **35 problèmes** dans la liste fermée — **5 P0, 15 P1, 15 P2**. Les identifiants restent stables : l'ex-P0-6 garde son numéro, déplacé en section P1.
...
- **SessionHistoryScreen** (253 l.) : liste saine côté dates (toDateKey partout) — mais **ex-P0-6, rétrogradé P1** (« min » faux sur un chemin étroit) et titre doublé (P2-11).
...
### P0 — mensonges visibles / pertes de données (5)
...
- **P0-2 · Le graphe « Ta forme sur 7 jours » est fabriqué deux fois.** (a) Chaque barre manquante est bouchée avec le TSB **du jour** (`tsbHistory[idx] ?? tsb`, `ProfileScreen.tsx:570`) : compte neuf = 7 barres identiques à +3 (le rebuild sème `[+3]` et le `?? tsb` remplit les 6 autres — des « jours » antérieurs à l'installation) ; historique partiel = des « jours passés » qui affichent la valeur d'aujourd'hui. (b) L'axe « J…J-6 » ment **en permanence** : même après `rebuildLoad`, la série est par jour **actif**, pas par jour calendaire — un joueur Lun/Jeu voit dimanche la valeur de jeudi étiquetée « J-1 ». Vrai pour tout joueur avec des jours de repos, c'est-à-dire tous. Nuances du re-échantillonnage : les doublons par événement issus du feedback sont corrigés en quelques secondes en ligne (watcher → rebuild) ; ceux des charges externes persistent jusqu'au prochain lancement. **Sous-grief retiré** : le `?? 0` du graphe d'intensité voisin est un zéro **vrai** (`dailyApplied` est autoritaire : un jour sans entrée a réellement porté zéro charge trackée) — la « Moy » est une moyenne hebdomadaire légitime, pas une dilution.
...
- **P0-4 · « Tests ce mois » / trophée « Tests du mois » ne comptent aucun test terrain.** Ils comptent des séances FKS « VMA-like » par heuristique sur les exercices (modality run / nom contenant « vma » / focus speed, intensité ≥ moderate — un footing modéré compte comme un « test »). `ProfileScreen.tsx:288,535`, `streakStats.ts:29-36,70-76`. Les vrais tests sont lus dans le même fichier (`testsCount`, `:129-133`) mais pas utilisés ici. 3 batteries faites ce mois → « 0 test ». **Aggravant relevé au re-échantillonnage** : la carte « Fais tes tests » (basée sur le vrai `testsCount`) cohabite sur le même écran — le joueur qui obéit à la relance voit ensuite « Tests ce mois : 0 » juste en dessous, contradiction visible en un seul scroll.
...
### P1 — réglages inertes visibles, données périmées, divergences réelles (15)

- **P0-6 (rétrogradé P1 au re-échantillonnage) · L'historique peut afficher une charge sRPE suffixée « min ».** Le défaut de code est exact : sans `durationMin`, repli sur `${Math.round(volumeScore)} min` (`SessionHistoryScreen.tsx:124-129`) — une valeur affichée « UA » en preview (`SessionPreviewScreen.tsx:161-162`), soit « ~315 min » pour 45 minutes réelles. Mais la portée réelle est étroite : les séances non validées (zombies) rendent un sous-titre **sans durée** (`:170-177`, jamais les fausses minutes) ; les séances restaurées depuis Firestore n'ont pas de `volumeScore` persisté et affichent « — » ; le seul déclencheur est un joueur qui **vide volontairement** le champ durée du feedback, pourtant toujours pré-rempli (`sessionSchema.ts:91` garantit `duration_min`), option que l'UI autorise (« laisse le champ vide », `useFeedbackSave.ts:131-133`). Mensonge réel mais rare et auto-infligé — correctif trivial (supprimer le repli ou afficher « — »).
...
- Workflow 3 (`wf_0d88eac3-7c1`, 07/08, réserve de méthode) : re-échantillonnage de P0-2 / P0-4 / P0-6 par 3 contre-vérificateurs frais à charge **inversée** (mission : réfuter). Résultat : P0-2 confirmé (2 nuances de portée, 1 sous-grief blanchi), P0-4 confirmé et aggravé, P0-6 rétrogradé P1. La passe prouve que la méthode mord quand on la braque.
- Contre-vérifications orchestrateur (⊙) : params de route morts ; badges héro Réglages ; titre Historique ×2. (Un 4e constat ⊙ — « Moy diluée par les `?? 0` du graphe d'intensité » — a été **infirmé** par le workflow 3 : ces zéros sont vrais, `dailyApplied` étant autoritaire.)
