...
### Pivot B2C joueur
- **Espace club/coach retire du parcours (2026-09)** : plus de choix d'espace, d'entree « Je suis coach », de code club, d'ecran coach, d'effectif ni de directive club cote app. La racine (`navigation/RootNavigator.tsx`) ne lit plus `users/{uid}.clubId` ni `clubs/*` ; le questionnaire n'ecrit jamais `clubId` (un rattachement historique reste intact en base). Un ancien compte coach (`profileCompleted: true` sans champs joueur) est renvoye completer son profil joueur (`domain/playerProfile.ts`). Les cles locales de l'ancien espace sont `LEGACY_*` dans `constants/storage.ts` (purge seulement). Les Cloud Functions et `firestore.rules` sont conservees telles quelles (suppression de compte, anciennes donnees).
- **Entrainements club et matchs du joueur CONSERVES** : `clubTrainingDays`/`matchDays` du profil restent la base du planning, des charges auto et du contexte de generation — ce sont des donnees sportives personnelles, pas une organisation de club.
- **Mode Coach retire** : 100% focus joueur, plus de Dashboard coach / coach players data
- **Profil / Parametres fiabilises (2026-09)** : chaque reglage visible produit son effet. Retires de l'ecran (valeurs persistees conservees, champs `@deprecated`) : Mode prive, unites km/mi et kg/lb, badge « Verifie », selecteur de strategie de rappel (RoutineScreen), sons en natif (web seulement). Analytics : `services/analytics.ts` applique l'opt-out Amplitude des l'init (preference restauree) et `resetSettings` PRESERVE `privacyAnalytics`. Notifications : une seule porte `applyNotificationPreferences` (services/notifications.ts) — permission telephone / preference joueur / rappels programmes distingues, token push best-effort jamais bloquant, applications serialisees. Export = « donnees locales » (utils/exportLocal.ts, inclut Mon corps, pas une sauvegarde). Parametres en 5 groupes (Mon compte / Notifications / Preferences de l'application / Donnees et confidentialite / Aide et informations) ; Profil : actions « Modifier mon profil » + Parametres sous l'identite, liens vers Progression/Historique/Tests/Mon corps, blocs « Ta forme » et « Ta regularite » retires (doublons de Progression).
