...
### D12 — complément après contre-vérification du lot 1 (01/09, nuit)

La contre-vérification adversariale du lot 1 a relevé que D12 exigeait « effet sur les pools à MESURER avant merge » et qu'aucune mesure n'existait. **Décision de l'orchestrateur : la mesure n'est pas applicable côté front et son abandon est acté.** Motif : le format du payload (`pains[]` + `injury_max_severity`) est strictement inchangé ; seuls changent (a) la population des zones envoyées (celles au statut actif / en reprise au lieu de celles déclarées dans les 7 derniers jours) et (b) leur durée (jusqu'à mise à jour par le joueur au lieu d'une expiration silencieuse). Le seul risque matériel identifié n'est pas un appauvrissement des pools mais **un refus RF2 permanent** pour une gêne laissée en gravité 3 — traité au round 2 du lot 1 par trois gestes : la phrase de sortie du refus dit vrai selon le drapeau (RF2 → « tant que tu n'as pas mis à jour cette gêne dans Mon corps »), un bouton « Ouvrir Mon corps » sur tout refus de sécurité, et une ligne d'aide sous la gravité 3 (« Pas de séance tant que cette gêne reste à ce niveau. »). La sentinelle #2 (« payload identique ») est retirée ; la sentinelle #7 (une gêne de 30 jours sans réponse reste dans `pains[]`) est conservée et verte.

Restes P3 acceptés au lot 1 (consignés, non bloquants) : une gêne en cours + douleur ≥ 3 au feedback affiche 3 boutons sans « Plus tard » ; la route Mon corps est montée dans le navigateur de setup sans y être atteignable ; coquille « sceance » dans le message du commit d3c70b7.
