...
// sort en SUCCES. Ce fichier s'execute avec la config dediee du depot :
...
function textes(instance: ReactTestInstance): string[] {
  return instance
    .findAllByType(Text)
    .flatMap((n) =>
      React.Children.toArray(n.props.children as React.ReactNode).filter((c): c is string => typeof c === "string")
    );
}
  ...
  it.each(casPret.map((f) => [f.id, f] as const))(
    ...
    (_id, fixture) => {
      ...
      const libelle = textesDeLaLigne.find(
        (n) =>
          React.Children.toArray(n.props.children as React.ReactNode).filter((c) => typeof c === "string").join("") ===
          vm.resume.libelle
      );
