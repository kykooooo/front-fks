...
| **1** | **Simuler** la migration des accès | `node lib/coachAccessBackfillCli.js --clubId=<club>` | N'écrit rien. On lit les compteurs et on vérifie qu'ils ressemblent à l'effectif réel. |
| **2** | **Appliquer** la migration, un club à la fois | `node lib/coachAccessBackfillCli.js --clubId=<club> --apply --je-confirme` | Pose `not_required` (majeurs déclarés) ou `pending` (mineurs / âge inconnu). **Jamais `approved`.** Fait AVANT le reste : sinon le coach perd tout son effectif d'un coup. |
