...
/**
 * LE TRAITEMENT, extrait du wrapper `onCall` pour les tests d'enveloppe
 * (functions/tests/callableEnvelope.test.ts). Identite lue ICI depuis
 * `request.auth` : c'est la callable la plus destructrice du dossier, et le seul
 * uid qu'elle touche est celui du jeton — jamais un champ de `request.data`.
 */
...
export const deleteAccount = onCall({ region: REGION }, deleteAccountHandler);
