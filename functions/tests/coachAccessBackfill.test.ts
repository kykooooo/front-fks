...
//
// Le BORNAGE du parcours (plafond, curseur, compteur attendu) a son propre
// fichier : functions/tests/coachAccessBackfillBornes.test.ts. Ici, le plafond
// est simplement fourni assez large pour ne jamais mordre — ce fichier parle du
// METIER, pas du parcours.

import {
  comparerMemberRefs,
  runCoachAccessBackfill,
  type CoachAccessBackfillStats,
...
/** Plafond volontairement large : ce fichier ne teste pas le bornage. */
const LARGE = 100;
...
/** Les compteurs d'un parcours accepte. Un refus ferait echouer le test ici. */
async function statsDe(
  store: CoachAccessBackfillStore,
  opts: { apply?: boolean; clubId?: string } = {},
): Promise<CoachAccessBackfillStats> {
  const res = await runCoachAccessBackfill(store, { limite: LARGE, ...opts });
  if (!res.ok) throw new Error(`parcours refuse : ${res.motif}`);
  expect(res.limiteAtteinte).toBe(false);
  return res.stats;
}
...
describe("runCoachAccessBackfill — simulation (mode par defaut)", () => {
  it("n'ecrit RIEN, mais annonce exactement ce qui serait ecrit", async () => {
    const m = monde();
    const stats = await statsDe(magasin(m));
  ...
  it("le ciblage par club borne reellement le parcours", async () => {
    const m = monde();
    const stats = await statsDe(magasin(m), { clubId: "clubB" });
...
describe("runCoachAccessBackfill — application", () => {
  it("pose le champ manquant selon la politique du club, sans fabriquer d'autorisation", async () => {
    const m = monde();
    await statsDe(magasin(m), { apply: true });
  ...
  it("un club en approval_required ne devient PAS consultable par ce script", async () => {
    const m = monde();
    await statsDe(magasin(m), { apply: true, clubId: "clubB" });
  ...
  it("est idempotent : un second passage n'ecrit plus rien", async () => {
    ...
    const stats = await statsDe(magasin(m), { apply: true });
  ...
  it("membership disparu entre l'inventaire et le traitement : compte, ne casse pas", async () => {
    ...
    // "parti" se glisse a sa place dans l'ordre (apres "joueur2", avant
    // "retire") : l'inventaire reste strictement croissant, sinon le coeur le
    // refuserait — et ce test-ci parle du membership disparu, pas de l'ordre.
    store.listPlayerMembers = async (parcours: ParcoursMembres) =>
      [...(await original(parcours)), { clubId: "clubA", playerUid: "parti" }].sort(
        comparerMemberRefs,
      );
    const stats = await statsDe(store, { apply: true });
