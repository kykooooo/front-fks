...
describe("garde solo — un exo à 2 reçu du serveur n'est jamais affiché nu", () => {
  test("équivalent solo disponible : str_nordic → Nordic curl assisté par bande, tracé dans les notes, dosage moteur conservé", () => {
    ...
    // NOM RECALÉ AU MERGE (12/08) — assertion NON affaiblie. Le libellé servi
    // n'est plus « Nordic assisté élastique » : fix/bibliotheque-precision fait
    // primer le nom éditorial V2 sur le nom front (applyEditorialContent, dans
    // engine/exerciseBank.ts). On épingle donc le nom réellement servi ET on
    // exige qu'il vienne de la fiche de banque : la garde sert un vrai
    // exercice, jamais un libellé fabriqué ni l'id brut.
    expect(ex.name).toBe("Nordic curl assisté par bande");
    expect(ex.name).toBe(EXERCISE_BY_ID["str_nordic_assisted_band"].name);
    // Et ce n'est pas l'exo à 2 de départ, simplement re-étiqueté.
